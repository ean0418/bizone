...
<div class="content">
    <div id="sidebar">
        <div id="sidebar-content">
            ...
            <button class="btn btn-primary" data-toggle="modal" data-target="#favoriteModal">
                관심 지역
            </button>
        </div>
    </div>
...
<script>
    ...
    $('#favoriteBtn').on('click', function () {
        ...
        if (selectedAdminCode && selectedServiceCode) {
            ...
            const rankIndex = $('#rank').text();  // 파워랭킹
    ...
    // 관심 지역 모달이 열릴 때, AJAX로 관심 지역 데이터를 가져오는 부분
            ...
            success: function (data) {
                ...
                if (data.length > 0) {
                    data.forEach(function (item, index) {
                        const successProbability = item.bs_success_probability !== undefined ? item.bs_success_probability + "%" : "정보 없음";
                        const rankIndex = item.rank_index !== undefined ? item.rank_index : "정보 없음";
                        const formattedDate = new Date(item.bia_date).toLocaleString();  // 타임스탬프를 로컬 시간으로 변환

                        // 번호는 index로 생성
                        const listItem = `
                    <tr>
                        <td>${index + 1}</td>  <!-- 번호 표시 -->
                        <td>${item.ba_name || '정보 없음'}</td>
                        <td>${item.bb_name || '정보 없음'}</td>
                        <td>${successProbability}</td>
                        <td>${rankIndex}</td>
                        <td>${formattedDate}</td>
                        <td><button class="btn btn-danger btn-sm removeFavoriteBtn" data-ba-code="${item.ba_code}" data-bb-code="${item.bb_code}">🆗</button></td>
                    </tr>`;
    ...
    // 찜 목록에서 삭제하는 함수
    function removeFavoriteItem(baCode, bbCode) {
        $.ajax({
            url: '/api/bizone/removeFavorite',  // 찜 삭제 API
            method: 'DELETE',
            contentType: 'application/json',
            data: JSON.stringify({
                bm_id: globalLoggedInUserId,  // 로그인한 사용자 ID
                ba_code: baCode,
                bb_code: bbCode
            }),
            success: function () {
                ...
                $('#favoriteModal').modal('show');  // 모달을 다시 열어서 새로고침
            },
            error: function (error) {
                console.error('찜목록에서 삭제하는 중 오류가 발생했습니다.', error);
                alert('찜목록에서 삭제하는 중 오류가 발생했습니다.');
            }
        });
    }
...
<%--            },--%>
<%--            success: function (data) {--%>
<%--                console.log(data);  // 데이터를 확인해보세요--%>
<%--                const favoriteList = $('#favoriteList');--%>
<%--                favoriteList.empty();  // 기존 목록 초기화--%>

<%--                if (data.length > 0) {--%>
<%--                    data.forEach(function (item) {--%>
<%--                        console.log("Item: ", item);  // 개별 아이템 확인--%>
<%--                        const successProbability = item.bs_success_probability !== undefined ? item.bs_success_probability + "%" : "정보 없음";--%>
<%--                        const rankIndex = item.rank_index !== undefined ? item.rank_index : "정보 없음";--%>
<%--                        const formattedDate = new Date(item.bia_date).toLocaleString();  // 타임스탬프를 로컬 시간으로 변환--%>
<%--                        const listItem = `--%>
...
<%--                    });--%>
<%--                } else {--%>
<%--                    favoriteList.append('<tr><td colspan="5">찜한 항목이 없습니다.</td></tr>');--%>
<%--                }--%>
<%--            },--%>
<%--            error: function (error) {--%>
<%--                console.error('관심 지역 목록을 불러오는 중 오류가 발생했습니다.', error);--%>
...
<%--        <div class="modal-content">--%>
<%--            <div class="modal-header bg-warning text-white">--%>
<%--                <h5 class="modal-title" id="favoriteModalLabel"><i class="fas fa-heart"></i> 찜 목록</h5>--%>
<%--            </div>--%>
<%--            <div class="modal-body">--%>
